 ...
 * Kidschool Breadcrumbs

 * Since ver 1.0

 * Add this to any template file by calling kidschool_breadcrumbs()

 */

 if(!function_exists('kidschool_breadcrumbs')){



	function kidschool_breadcrumbs(){
		...
		$breadcrumbs_separator = $kidschool_opt['kidschool_breadcrumbs_separator'];
		...
		if($breadcrumbs_separator){
			...
			$breadcrumbs_separator = "<li>/</li>";

		}



		$breadcrumbs_cat = $kidschool_opt['kidschool_breadcrumbs_post_cat'];



	  /* === OPTIONS === */

		$text['home']     = esc_html__('Home' , 'kidschool'); // text for the 'Home' link

		$text['category'] = esc_html__('Archive by Category "%s"' , 'kidschool'); // text for a category page

		$text['tax'] 	  = esc_html__('Archive for "%s"' , 'kidschool'); // text for a taxonomy page

		$text['search']   = esc_html__('Search Results for "%s" Query' , 'kidschool'); // text for a search results page

		$text['tag']      = esc_html__('Posts Tagged "%s"' , 'kidschool'); // text for a tag page

		$text['author']   = esc_html__('Articles Posted by "%s"' , 'kidschool'); // text for an author page

		$text['404']      = esc_html__('Error 404' , 'kidschool'); // text for the 404 page
		...
		if (is_home() || is_front_page()) {
			...
			if ( get_query_var('paged') ) {
				...
				esc_html_e('Page','kidschool') . ' ' . get_query_var('paged');
